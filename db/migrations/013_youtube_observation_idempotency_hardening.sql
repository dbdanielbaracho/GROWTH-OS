-- Issue #26: forward-only hardening of the YouTube observation idempotency contract.
-- Migration 010 is intentionally left immutable after validation. This replacement tightens
-- conflict detection so one idempotency key cannot silently alias materially different facts.
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION '013 failed: youtube_record_metric_observation ownership/security boundary changed';
  END IF;

  IF has_function_privilege('public',fn::text,'EXECUTE') THEN
    RAISE EXCEPTION '013 failed: PUBLIC unexpectedly has EXECUTE on youtube_record_metric_observation';
  END IF;

  IF NOT has_function_privilege('app_runtime',fn::text,'EXECUTE') THEN
    RAISE EXCEPTION '013 failed: app_runtime lacks required helper EXECUTE';
  END IF;
END $$;

CREATE OR REPLACE FUNCTION growth.youtube_record_metric_observation(
...
BEGIN
  ...
  IF existing.id IS NOT NULL THEN
    ...
    -- those are retry-time policy metadata. Everything below is stable factual/source identity
    -- for one logical collection run and must remain identical under the same idempotency key.
    IF ROW(
