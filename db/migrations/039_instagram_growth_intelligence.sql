...
DECLARE
  ...
  v_logic_version constant text := 'growth.instagram.likes-acceleration.v1';
  v_confidence jsonb;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'growth intelligence requires connected authorized Instagram account';
  END IF;
  ...
  v_confidence := jsonb_build_object(
    'level', CASE
      ...
      ELSE 'medium'
    END,
    'method', 'latest like_count vs mean of previous complete observations',
  ...
  INSERT INTO growth.insights (
    ...
    'confirmed_account',
    format(
      'Instagram likes are %s%% above this account''s recent baseline.',
...
ALTER FUNCTION growth.recompute_instagram_growth_intelligence(uuid) OWNER TO growth_migrator;
REVOKE ALL ON FUNCTION growth.recompute_instagram_growth_intelligence(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION growth.recompute_instagram_growth_intelligence(uuid) TO app_runtime;
