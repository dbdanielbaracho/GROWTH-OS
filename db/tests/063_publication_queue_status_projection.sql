-- Publication queue status projection contract gate.
\set ON_ERROR_STOP on

BEGIN;

DO $queue_status_gate$
DECLARE
  status_oid oid;
  status_def text;
  helper_owner text;
...
BEGIN
  ...
  IF status_oid IS NULL THEN
    RAISE EXCEPTION '063 failed: publication queue status helper is missing';
  END IF;
  ...
  IF position('security definer' IN lower(status_def)) = 0
     ...
     OR position('queue_state' IN lower(status_def)) = 0
     OR position('j.state' IN lower(status_def)) = 0
     OR position('j.attempts' IN lower(status_def)) = 0
     OR position('payload' IN lower(status_def)) > 0
     OR position('service_principal_id' IN lower(status_def)) > 0
     OR position('leased_until' IN lower(status_def)) > 0
  THEN
    RAISE EXCEPTION '063 failed: queue status helper projection is incomplete or unsafe';
  END IF;
  ...
  IF helper_owner <> 'growth_migrator' OR is_definer IS DISTINCT FROM true THEN
    RAISE EXCEPTION '063 failed: queue status helper owner/SECURITY DEFINER boundary';
  END IF;
  ...
  THEN
    RAISE EXCEPTION '063 failed: queue status projection privileges are not least-privilege';
  END IF;
END;
$queue_status_gate$;
...
SELECT 'TEST-063 PASS: publication queue status projection contract' AS result;
