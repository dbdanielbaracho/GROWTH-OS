...
**Achado de promoção após o merge do PR #209:** o PR CI #1367 passou integralmente e o PR foi mesclado como `032c160a23df7138da2c0b17f4dfe4c3050e9b92`. No primeiro deploy Railway, o migrator encerrou com status superficial `SUCCESS`, mas seu log registrou `Unregistered production migrations: 073_youtube_connection_revocation.sql`; portanto a migration não foi tratada como aplicada e o botão não foi declarado disponível. A causa foi a ausência da migration 073 na lista explícita de reconciliação de `db/scripts/apply-production-migrations.mjs`, lacuna que o release-hardening anterior também não exigia.

**Correção candidata:** registrar 073 no reconciliador com detecção física não vacuosa dos dois helpers, do `DELETE` da credencial, do upsert seguro para reconexão e dos grants runtime/PUBLIC; exigir migration/gate 073 no release-hardening. Validação local da correção: sintaxe Node PASS, Release Hardening PASS e `git diff --check` PASS. Próximo gate: PR corretivo, CI verde, merge e nova promoção Railway com log explícito `Applied migration: 073_youtube_connection_revocation.sql` antes do aceite.


**Aceite final da entrega do botão:** PR #210 passou CI #1369 e foi mesclado como `b1d86b41311a5400d682746aca42b1c2253eded6`; o CI de `main` #1370 também terminou `SUCCESS`. No Railway canônico, migrator `a79e97aa-5c1c-4a41-93ae-239a5338a4c8` terminou `SUCCESS` e registrou literalmente `Applied migration: 073_youtube_connection_revocation.sql`; publication worker `8428e157-54d2-4d57-b99c-263b13cd93c0` terminou `SUCCESS`. O primeiro runtime web com o botão foi o PR #209, SHA `032c160a23df7138da2c0b17f4dfe4c3050e9b92`, deploy `df1a97d8-f195-48be-8716-4c670012d25e`; após a promoção do PR #210, o runtime final aceito passou a SHA `b1d86b41311a5400d682746aca42b1c2253eded6`, deploy `738e8489-c284-4fd4-bfad-166b49268a4f`, também `SUCCESS`. Probes públicos retornaram readiness `status=ready` e `database=ok`, deployment identity no mesmo runtime e o bundle servido contém `Revoke connection`. Resultado: o botão de revogação YouTube está implementado, testado e disponível em produção; a reconexão humana Google e o sync real de sete dias permanecem como o próximo gate externo, não como pendência desta implementação.
